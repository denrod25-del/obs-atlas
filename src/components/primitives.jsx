/* Shared primitives — obs-atlas. Sky-300 primary, amber-400 accent. */

import { useState } from 'react';
import { AlertTriangle, CheckCircle2, Info, XCircle, Lightbulb, Radio } from 'lucide-react';

export function Kbd({ children }) {
  return (
    <code className="px-1.5 py-0.5 text-[12.5px] bg-zinc-800/70 text-sky-300 border border-zinc-700"
...
export function H2({ num, children }) {
  return (
    <h2 className="text-zinc-100 mt-12 mb-4 flex items-baseline gap-3"
      ...
      <span className="text-sky-300 text-[12px] tracking-[0.25em] uppercase shrink-0"
        style={{ fontFamily: 'JetBrains Mono, monospace' }}>
        {num}
      </span>
      <span>{children}</span>
...
export function Code({ id, lang = 'text', children }) {
  ...
  return (
    <div className="my-4 border border-zinc-800">
      <div className="flex items-center justify-between bg-zinc-950 px-3 py-1.5 border-b border-zinc-800 text-[10px] uppercase tracking-[0.2em] text-zinc-500"
        ...
        <button onClick={copy} className="hover:text-sky-300 transition-colors">
          {copied ? 'copied' : 'copy'}
        </button>
      </div>
...
const CALLOUT_STYLES = {
  tip:    { icon: Lightbulb,    accent: 'text-amber-400',  border: 'border-amber-400/40',  bg: 'bg-amber-400/5' },
  warn:   { icon: AlertTriangle,accent: 'text-amber-400',  border: 'border-amber-400/40',  bg: 'bg-amber-400/5' },
  info:   { icon: Info,         accent: 'text-sky-300',    border: 'border-sky-300/40',    bg: 'bg-sky-300/5' },
  win:    { icon: CheckCircle2, accent: 'text-lime-400',   border: 'border-lime-400/40',   bg: 'bg-lime-400/5' },
  dont:   { icon: XCircle,      accent: 'text-rose-400',   border: 'border-rose-400/40',   bg: 'bg-rose-400/5' },
  signal: { icon: Radio,        accent: 'text-sky-300',    border: 'border-sky-300/50',    bg: 'bg-sky-300/10' },
};
...
export function SectionLabel({ children }) {
  return (
    <div className="my-8 text-sky-300 text-[10px] tracking-[0.4em] uppercase border-b border-zinc-800 pb-2"
      style={{ fontFamily: 'JetBrains Mono, monospace' }}>
      ── {children} ──
    </div>
  );
